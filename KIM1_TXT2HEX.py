...
***This script requires Python3.9 or greater***
"""
)

input_file = input(
    "Enter the name of the text file generated by the Retroassembler (Omit the .txt): "
)
print("\n")
...
txt_content: list[str] = [arg for line in lines for arg in line.split()]
...
while i < len(txt_content):
    if len(txt_content[i]) == 4 and int(txt_content[i]) % 10 != 0:
        txt_content.pop(i)  # Remove invalid address
...
i = 0
while i < len(txt_content):
    if len(txt_content[i]) == 4:
        txt_content.insert(i + 1, "00")  # Insert "00" after the address
...
i = 0
while i < len(txt_content):
    if len(txt_content[i]) == 4:  # Check if the current item is an address
        ...
        end_index = min(len(txt_content), start_index + 16)  # Max 16 bytes per line
        ...
        txt_content.insert(i, byte_count_marker)
...
for line in lines:
    ...
    start_addr = line[1]
    checksum += (
        int(byte_count[1:], 16) + int(start_addr[0:2], 16) + int(start_addr[2:], 16)
    )
...
print(
    "Please copy and paste this formatted Intel Hex into your terminal using paste special:\n"
)
print(f"{kim1_lines}\n")
...
1. File input
2. File output
assert sys.version >= (3, 9), f"Descriptive error message and what python version was found"
3. Defult output is stdout
    `out_file = sys.stdout if output_file_path else open(output_file_path, 'w')`

4. Use Black Formatter, vs code has an exenstion
4. Rust the entire program
    """
